...
TIP: link:envcombo-ctl.html[envcombo-ctl]'s Capture/Export section automates
this entire dance over SSH -- one click writes `regs.bin` or a buffer
capture straight to a local path, no listener required. The manual `nc`
workflow below is still useful to understand what's happening on the wire,
...
# See user-guide § "Power-mode logic" for the full consumer table.
...
To capture a different state (e.g. the Continuousgit baseline), disable the
consumer (`echo 0 > events/in_illuminance_thresh_either_en`) and repeat
from Step 1.

.ImHex decoding a register snapshot via `docs/imhex/envcombo-als.hexpat` -- here the Continuous baseline (`pwr_mode = Continuous`, a 100/800 threshold window)
...
Capturing the register file afterwards and decoding it in ImHex shows the
result of exactly that write -- `cfg.raw = 0x84`, i.e. `als_en = 1` at the
default gain x1 / 200 ms (the `Reg8<CFG>` union shows raw byte and decoded
fields side by side):

.The byte written above, decoded -- `cfg.raw = 0x84` alongside the bitfield (`als_en = 1`, `als_gain`, `als_time`)
...
Decoding a capture in ImHex shows the matching result: the `be u16 als_out`
field comes back MSB-first as `0x00F0` (240) -- the `0x00` MSB and the `0xF0`
LSB are exactly the two data bytes clocked out in the waveform above.

.The 16-bit `als_out` read decoded MSB-first -- `0x00F0` from registers `0x04-0x05` (the `0xF0` LSB clocked out last)
