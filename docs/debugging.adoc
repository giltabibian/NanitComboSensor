...
Before decoding the bytes at rest it helps to see them in motion. The
driver never touches a bus directly -- it issues four `i2c_smbus_*`
primitives (link:../recipes-kernel/envcombo-driver/files/envcombo.c[`envcombo.c`],
...
decodes those fields MSB-first.
...
The four diagrams are PlantUML timing sources under `docs/diagrams/`
