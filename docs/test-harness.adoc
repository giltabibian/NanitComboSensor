...
image::diagrams/svg/test-stack.svg[Test stack block diagram]
...
The 15 tests run one at a time (never concurrently); the exit code is 0 only
if every test passes.

The *order*, though, is deliberately randomized rather than fixed:
`module-loading` always runs first and `module-unloading` /
`kernel-log-health` always run last, but the device tests in between are
shuffled by a seeded PRNG on every run. Each device test calls
`reset_baseline()` first (and the configuration tests restore the defaults
they started from), so the suite is order-independent by construction -- the
shuffle is what actively proves that isolation holds, rather than the suite
quietly depending on a fixed order. The seed is printed at startup and can be
pinned with `--seed=N` or `ENVCOMBO_TEST_SEED=N` to replay a specific order
deterministically. The numbering in the table below is the source order in
which the tests are listed, not the order they run in.
...
| Both modules present (`modprobe` fallback if not autoloaded); debugfs
  ...
  probe; probe also provisioned the registers -- INT_CFG = enabled+latched
  (0xC0) and CFG = ALS-on at the default gain/time (0x84).
...
| `_available` lists exactly the datasheet values; a valid gain write is
  ...
  scale is checked to be 1/gain for all four gains; a valid time write lands
  in CFG; unsupported values (gain 3, time 0.075 s) are rejected with state
  untouched; defaults restored.

| 4 | `gain-response`
| The *functional* counterpart to `gain-and-integration-time`: sweeps all
  four gains with live one-shot reads and asserts (a) the raw value grows
  with gain -- catching a gain that is ignored or inverted -- and (b) the
  gain-compensated value `raw \* scale` is gain-independent, i.e. the
  reported scale actually undoes the gain. Degenerate ramp positions (signal
  at zero or wrapping mid-sweep) are retried so only a real fault fails.

| 5 | `time-response`
| The integration-time analogue of `gain-response`: the simulator divides the
  reading by `time_ms / 50`, so sweeping the four integration times must make
  raw strictly *decrease*, while `raw \* (time / 50)` stays constant -- proving
  the time setting changes the measurement and not just the CFG bits. Same
  retry-on-degenerate-ramp handling as the gain sweep.

| 6 | `factory-gain-cal`
| Injects a `CAL_AGAIN` factory gain multiplier via debugfs and reloads the
  driver so it re-reads the register at probe; the reported scale must fold in
  the multiplier (effective gain = configured gain x CAL_AGAIN) at gains 1 and
  4. The override is always undone and the driver reloaded afterwards.

| 7 | `factory-atime-override`
| Injects a non-zero `CAL_ATIME` via debugfs and reloads the driver: the
  integration time must be forced to the factory value, the `_available` list
  collapsed to that single value, and `integration_time` made read-only (a
  write is refused and changes nothing). Always restored afterwards. Exercises
  the factory-override branch that is otherwise only triggered by hand.

| 8 | `threshold-attributes`
| Threshold writes land in the big-endian TH_LOW/TH_HIGH register pairs;
  ...
  unchanged after the rejected writes; the boundary case low == high (a
  degenerate window) is accepted and lands in the registers.
...
| 11 | `buffer-no-timestamp`
| The other scan layout: with the timestamp scan element *disabled*, records
  must be bare 2-byte little-endian light samples (the layout the docs'
  ImHex line_plot relies on). An even byte count and every u16 in range pin
  the record size -- a leaked 16-byte timestamped record would read back as
  wildly out-of-range values.

| 12 | `buffer-timestamp-accuracy`
| The *absolute* timestamp, complementing the spacing check in
  `triggered-buffer`: the device's `current_timestamp_clock` is mapped to the
  matching POSIX clock, the buffer read is bracketed with `clock_gettime()`
  on that clock, and every fresh sample's stamp must be non-zero and land
  inside that wall-clock window -- catching a wrong clock source, a
  stuck/zero stamp, or a constant offset a delta-only check cannot see.

| 13 | `power-fsm-transitions`
...
[10/15] triggered-buffer            FAIL: median sample spacing 612 ms, expected ~200 ms
----
...
When a failure looks order-dependent, replay the exact run using the seed
printed in the run's banner:
