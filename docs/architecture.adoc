...
The diagram shows two distinct communication paths between the device and the
SoC:

* *I2C bus* (SDA + SCL) -- the driver's only active path; all register reads
  and writes go here.
* *INT pin* -- a dedicated physical wire, separate from I2C, that the device
  asserts to interrupt the CPU. The driver never reads or writes this pin;
  the SoC's interrupt controller detects its edge and wakes the IRQ thread.
...
<<design-decisions>>).
+
Note the intentional opposition: STATUS bits go `0 -> 1` when an event fires
(register flag convention: `1` = yes), while the physical INT pin goes
`HIGH -> LOW` (active-low electrical convention). They are two different
representations of the same event -- a bit value and a voltage level -- and
they move in opposite directions by design.
+
----
Event fires
  STATUS bit: 0 → 1   (flag set: "event occurred")
  INT pin:    HIGH → LOW  (active-low pulse to the CPU)
----
...
The device has 19 byte-wide registers and the driver touches 13 of them (the
other 6 are temperature/humidity registers, out of scope here); a regmap
adds little. Decisive practical point: `CONFIG_REGMAP_I2C` is a
...
while nothing is consuming ALS data -- correct resource use that keeps the
device in low-power SLEEP mode until something needs it. Tradeoff:
