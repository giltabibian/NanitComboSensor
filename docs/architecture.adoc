...
....
                userspace
  +-----------------------------------------------+
  |  sysfs attrs        /dev/iio:deviceN           |
  |  (raw, scale,       (event fd via ioctl,       |
  |   gain, int_time,    buffered scan records)    |
  |   thresholds)                                  |
  +-----------------------+------------------------+
                          |
  +-----------------------v------------------------+
  |                   IIO core                      |
  |   channels / events / trigger / kfifo buffer    |
  +-----------------------+------------------------+
                          |  read_raw/write_raw, event cbs,
                          |  trigger ops, pollfunc
  +-----------------------v------------------------+
  |               envcombo driver                   |
  |  power-mode FSM | threaded IRQ | i2c_smbus xfer |
  +-----------+------------------------+-----------+
              | SMBus byte read/write  | INT (active-low)
  +-----------v------------------------+-----------+
  |        i2c-envcombo-sim (or real HW)            |
  |   ENV-COMBO device model @ 0x39, IRQ source     |
  +-------------------------------------------------+
....
...
LSB at base + 1. The driver accesses them with two SMBus byte transfers
(`envcombo_read_reg16()` / `envcombo_write_reg16()`).
...
....
  ...
  | ev_en_rising/ev_en_falling  event-enable state (sysfs)       |
...
* The threaded IRQ handler deliberately does *not* take the mutex: a raw
  ...
  handler snapshots `ev_en_*` / `thresh_*` / `buffer_en` with `READ_ONCE()`,
...
currently active consumers. Three boolean inputs drive the FSM:
`buffer_en` (triggered buffer running), `ev_en_rising`, `ev_en_falling`
(threshold events enabled). `envcombo_update_power_mode()` recomputes the
target mode whenever any of them changes; ONE-SHOT is a transient state used
only for on-demand raw reads while otherwise idle.

....
                       sysfs read of in_illuminance_raw
                       while idle (no buffer, no events)
            +--------------------------------------------------+
            |                                                  v
       +---------+        conversion done:              +-----------+
       |  SLEEP  |        HW latches ALS_OUT,           | ONE-SHOT  |
       |  0x01   |<------ sets ALS_RDY, auto-returns ---|   0x02    |
       | (reset  |        to SLEEP                      | transient |
       |  state) |                                      +-----------+
       +---------+
          |   ^
          |   |  last consumer disabled
          |   |  (!buffer_en && !ev_en_rising && !ev_en_falling)
          |   |
          v   |
      +--------------+
      |  CONTINUOUS  |   buffer enabled OR any threshold event enabled;
      |     0x03     |   device converts every cycle (200 ms),
      +--------------+   ALS_RDY (and possibly ALS_INT) each cycle
....
...
....
  envcombo_probe
     |
     |  reject if no IRQ wired / no SMBus byte-data support
     |  devm_iio_device_alloc, init mutex + completion
     |
     |  WHO_AM_I == 0xEB ?  -- no --> return -ENODEV
     |
     |  read CALIB_ALS_GAIN, CALIB_ALS_TIME (once)
     |     calib_atime != 0 --> dev_warn: integration_time is read-only
     |
     |  CFG       <- ALS_EN | gain=x1 | time=200ms
     |  INT_CFG   <- INT_EN | INT_LATCH          (POL=0, active-low)
     |  ALS_TH_LOW  <- 0x0000,  ALS_TH_HIGH <- 0xFFFF   (full range =
     |                                  thresholds effectively disabled)
     |  PWR_MODE  <- SLEEP     (stay low-power until first consumer)
     |
     |  devm_request_threaded_irq (NULL primary, IRQF_TRIGGER_LOW|ONESHOT)
     |  devm_iio_trigger_alloc/register  ("envcombo-devN")
     |  devm_iio_triggered_buffer_setup  (pollfunc stores timestamp)
...
....
 userspace            driver (read_raw)            IRQ thread          device
    |                       |                          |                  |
    |--- read raw --------->|                          |                  |
    |                       | mutex_lock               |                  |
    |                       | idle? yes                |                  |
    ...
    |                       |   (500 ms timeout        |                  |
    |                       |    -> -ETIMEDOUT)        |                  |
    |                       | read ALS_OUT MSB+LSB --------------------->|
    |                       | mutex_unlock             |                  |
    |<---- value -----------|                          |                  |
....

If CONTINUOUS is already active (buffer or events on), the function skips
the ONE-SHOT/completion block and reads the latched ALS_OUT directly.

=== Threshold event flow

Userspace configures `in_illuminance_thresh_rising_value` (-> ALS_TH_HIGH)
and/or `in_illuminance_thresh_falling_value` (-> ALS_TH_LOW), then enables
`..._thresh_rising_en` / `..._thresh_falling_en`. Enabling either event sets
`INT_CFG.INT_EN` and drives the FSM to CONTINUOUS.

....
 device (CONTINUOUS)            IRQ thread                       userspace
    |                               |                                 |
    | conversion crosses threshold  |                                 |
    | (in-range -> out-of-range):   |                                 |
    | ALS_RDY=1, ALS_INT=1,         |                                 |
    | INT pulses                    |                                 |
    |------------------------------>|                                 |
    |                               | read STATUS (clears + rearms)   |
    |              ALS_RDY: complete(als_done);                       |
    |                       buffer_en? -> iio_trigger_poll            |
    |              ALS_INT: read ALS_OUT, compare with                |
    |                       shadowed thresholds to recover            |
    |                       the direction the HW doesn't report:      |
    |                               |                                 |
    |                value >= thresh_high && rising_en                |
    |                  -> iio_push_event(THRESH, RISING) ------------>| event fd
    |                value <= thresh_low && falling_en                | (POLLIN,
    |                  -> iio_push_event(THRESH, FALLING) ----------->|  read
    |                otherwise: value already back in range           |  iio_event_data)
    |                  -> event dropped, not misreported              |
....

The hardware reports only "a threshold was crossed" via the single `ALS_INT`
bit. The driver disambiguates by reading the current sample and comparing it
against the configured thresholds, so userspace receives standard
direction-resolved rising/falling IIO events. The race where the value
returns in-range before the handler reads it results in a dropped event by
design (see <<design-decisions>>).

Threshold writes are validated: a rising value below the current falling
value (or vice versa) is rejected with `-EINVAL`, so the window stays
well-formed.
...
....
 userspace: scan_elements/in_illuminance_en=1 ; buffer/enable=1
    |
    v
 IIO core -> set_trigger_state(enable=1)
                |  buffer_en = true; FSM -> CONTINUOUS
                v
 device: conversion every 200 ms, ALS_RDY=1, INT pulses
                |
                v
 IRQ thread: read STATUS; ALS_RDY && buffer_en -> iio_trigger_poll(trig)
                |
                v
 pollfunc top half: timestamp = iio_get_time_ns()
                |
                v
 envcombo_trigger_handler (bottom half):
     read ALS_OUT MSB+LSB -> scan.light
     iio_push_to_buffers_with_timestamp(scan, pf->timestamp)
     iio_trigger_notify_done
                |
                v
 userspace: read /dev/iio:deviceN
     record: [ u16 light | 6B pad | s64 timestamp ]  (16 bytes)
....

Disabling the buffer calls `set_trigger_state(0)`; if no events are enabled
either, the FSM returns the device to SLEEP.
...
| `in_illuminance_integration_time` (+`_available`) | CFG[2:1] | One of `0.05 0.1 0.2 0.4` s; read-only (`-EOPNOTSUPP` on write) when CALIB_ALS_TIME override is set
| `events/in_illuminance_thresh_rising_value` | ALS_TH_HIGH | 0..65535; must stay >= falling value
| `events/in_illuminance_thresh_falling_value` | ALS_TH_LOW | 0..65535; must stay \<= rising value
| `events/in_illuminance_thresh_{rising,falling}_en` | INT_CFG.INT_EN + FSM | Either enables interrupts and CONTINUOUS mode
...
Matches the power-on default (SLEEP) and only spends power while a consumer
is active -- the behavior expected of an ALS in a battery-powered product.
Tradeoff: more state tracking (three flags + shared FSM helper) than a
naive "CONTINUOUS at probe" design, and the first raw read pays one
conversion of latency (bounded by the 500 ms completion timeout).
...
always coincides with new data. `IRQF_TRIGGER_LOW` matches `INT_POL=0`
(active-low), the power-on default the driver keeps.

Direction-resolved events from direction-less hardware::
The standard IIO threshold ABI is rising/falling, but the hardware reports
a single `ALS_INT` flag. Rather than pushing `IIO_EV_DIR_EITHER` (valid but
less useful to consumers), the handler reads the current sample and infers
the direction from the configured thresholds. If the value has already
returned in-range, the event is dropped rather than misreported -- for an
ambient-light alert, a stale notification about a condition that no longer
holds has negative value.
...
of the enable flags and thresholds instead, accepting a benign race window
around concurrent threshold rewrites.
...
The device is left in SLEEP whenever no consumer is active, which is also
the correct state to leave behind on unbind.
