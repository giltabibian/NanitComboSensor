...
| `scan_elements/in_illuminance_en` / `in_timestamp_en` and `buffer/length`
  ...
  live light-vs-sample plot. The plot polls `in_illuminance_raw` over plain
  sysfs every ~250ms rather than streaming `/dev/iio:deviceN` with `dd`:
  the IIO core only allows one opener of that chardev at a time, and a
  continuous `dd` would hold it for as long as the plot ran, permanently
  locking out the Events section's `envcombo-evtcat` (which needs the same
  chardev briefly, to bootstrap its event fd). Polling sysfs instead means
  the live plot and the event monitor can run at the same time.
...
| `envcombo-evtcat` must be on the target's `PATH` (installed by the
  ...
  threshold window is actually crossable from the current raw value. If a
  live event ever did arrive but the log then goes quiet permanently after
  a Stop/Start cycle with `Device or resource busy` showing up, that's a
  stale `dd` from the Capture/Export buffer capture (or, before this was
  fixed, the old `dd`-based live plot) still holding `/dev/iio:deviceN`
  open -- the Events section's own stray-process cleanup only ever targets
  `envcombo-evtcat`, never `dd`, so it won't clear that on its own; stop
  whatever started the `dd` first.
